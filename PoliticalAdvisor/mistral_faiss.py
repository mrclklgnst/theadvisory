...
def ms_build_faiss_programs(faiss_dir_path, bucket_name, faiss_dir, pdf_list):
    ...
    '''
    vector_store = initialize_ms_faiss()
    ...
    for pdf in pdf_list:
        ...
        logger.info(f"Adding {len(pdf_splits)} documents to FAISS index from {pdf}")
    ...
    try:
        ...
        # else try to reformat the answer
            ...
            try:
                ...
                logger.info('Response from MistralAI not in expected format')
                response["answer"] = answer_raw
    except:
        logger.info('Response from MistralAI not in expected format')
        response["answer"] = answer_raw
